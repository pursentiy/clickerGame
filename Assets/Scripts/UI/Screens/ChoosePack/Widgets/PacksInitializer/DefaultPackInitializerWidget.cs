...
namespace UI.Screens.ChoosePack.Widgets
{
    public class DefaultPackInitializerWidget : BasePackInitializerWidget
    {
        ...
        protected override Func<IDisposeProvider, IPromise<MediatorFlowInfo>> GetShowMessagePopupPromiseFunc(RectTransform popupAnchorRect)
        {
            return (disposeProvider) =>
            {
                ...
                var context = new MessagePopupContext(_localizationService.GetFormattedValue(LocalizationExtensions.AdsInfo, _gameInfoProvider.StarsRewardForAds), anchorRect, MessagePopupFontSize, spriteAsset);
