...
using Services;
using ThirdParty.SuperScrollView.Scripts.Common;
using UnityEngine;
using UnityEngine.EventSystems;
using UnityEngine.UI;

namespace ThirdParty.SuperScrollView.Scripts.StaggeredGridView
{
    [System.Serializable]
    ...
    */
    public class GridViewLayoutParam
    {
        ...
        public bool CheckParam()
        {
            ...
            {
                LoggerService.LogError("mColumnOrRowCount shoud be > 0");
                return false;
            }
            if (mItemWidthOrHeight <= 0)
            {
                LoggerService.LogError("mItemWidthOrHeight shoud be > 0");
                return false;
            }
            if (mCustomColumnOrRowOffsetArray != null && mCustomColumnOrRowOffsetArray.Length != mColumnOrRowCount)
            {
                LoggerService.LogError("mGroupOffsetArray.Length != mColumnOrRowCount");
                return false;
            }
            return true;
        }
    ...
    public class LoopStaggeredGridView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        public StaggeredGridItemPrefabConfData GetItemPrefabConfData(string prefabName)
        {
            foreach (StaggeredGridItemPrefabConfData data in mItemPrefabDataList)
            {
                ...
                {
                    LoggerService.LogError("A item prefab is null ");
                    continue;
                }
        ...
        */
            ...
            StaggeredGridViewInitParam initParam = null)
        {
            ...
            {
                LoggerService.LogError("layoutParam can not be null!");
                return;
            }
            ...
            if (mScrollRect == null)
            {
                LoggerService.LogError("LoopStaggeredGridView Init Failed! ScrollRect component not found!");
                return;
            }
            if (mDistanceForRecycle0 <= mDistanceForNew0)
            {
                LoggerService.LogError("mDistanceForRecycle0 should be bigger than mDistanceForNew0");
            }
            if (mDistanceForRecycle1 <= mDistanceForNew1)
            {
                LoggerService.LogError("mDistanceForRecycle1 should be bigger than mDistanceForNew1");
            }
            ...
            {
                LoggerService.LogError("ScrollRect.horizontalScrollbarVisibility cannot be set to AutoHideAndExpandViewport");
            }
            ...
            {
                LoggerService.LogError("ScrollRect.verticalScrollbarVisibility cannot be set to AutoHideAndExpandViewport");
            }
            ...
            {
                LoggerService.LogError("LoopStaggeredGridView.InitListView method can be called only once.");
            }
        ...
        public void ResetGridViewLayoutParam(int itemTotalCount, GridViewLayoutParam layoutParam)
        {
            ...
            {
                LoggerService.LogError("ResetLayoutParam can not use before LoopStaggeredGridView.InitListView are called!");
                return;
            }
        ...
        */
        ...
        void InitItemPool()
        {
            foreach (StaggeredGridItemPrefabConfData data in mItemPrefabDataList)
            {
                ...
                {
                    LoggerService.LogError("A item prefab with name " + prefabName + " has existed!");
                    continue;
                }
                RectTransform rtf = data.mItemPrefab.GetComponent<RectTransform>();
                if (rtf == null)
                {
                    LoggerService.LogError("RectTransform component is not found in the prefab " + prefabName);
                    continue;
                }
                AdjustAnchor(rtf);
                AdjustPivot(rtf);
        ...
        public void UpdateListView(float distanceForRecycle0, float distanceForRecycle1, float distanceForNew0, float distanceForNew1)
        {
            ...
            while (needContinueCheck)
            {
                ...
                {
                    LoggerService.LogError("UpdateListView while loop " + checkCount + " times! something is wrong!");
                    break;
                }
